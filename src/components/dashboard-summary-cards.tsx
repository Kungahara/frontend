...
function DashboardGraph({ products, sales }: { products: Product[]; sales: Sale[] }) {
  ...
  return <section className="dashboard-graph-panel" aria-label={view === "sales" ? "General sales graph" : "Income and expenses graph"}>
    <header><nav className="sales-view-tabs" aria-label="Dashboard graphs"><button className={view === "finance" ? "active" : ""} type="button" aria-pressed={view === "finance"} onClick={() => { setView("finance"); setHovered(null); }}>Income &amp; expenses</button><button className={view === "sales" ? "active" : ""} type="button" aria-pressed={view === "sales"} onClick={() => { setView("sales"); setHovered(null); }}>Sales</button></nav><Link className="dashboard-continue-button" href={view === "sales" ? "/sales" : "/finance"}>Continue to page <ArrowRight /></Link></header>
