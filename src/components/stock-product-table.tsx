...
function StockAnalysisChart({ products, sales, categoryId, productId, period, year, month, weekIndex }: { products: Product[]; sales: Sale[]; categoryId: string; productId: string; period: AnalysisPeriod; year: number; month: string; weekIndex: number }) {
  ...
  return <div className="stock-analysis-chart">
    {selected.length ? hasRecordedSales ? <svg viewBox="0 0 560 225" role="img" aria-label={`Recorded income and expenses for the selected ${period === "1Y" ? "year" : period === "1M" ? "month" : "week"}`} onMouseLeave={() => setHoveredPoint(null)}>
      <defs>
        <linearGradient id="income-area" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stopColor="var(--app-blue)" stopOpacity="0.2" /><stop offset="1" stopColor="var(--app-blue)" stopOpacity="0" /></linearGradient>
    ...
    {adding && <div className="stock-delete-backdrop" role="presentation">
      <form ref={addFormRef} className="stock-delete-dialog stock-add-dialog" role="dialog" aria-modal="true" aria-labelledby="add-product-title" aria-busy={busy} onSubmit={(event) => { event.preventDefault(); const submitter = (event.nativeEvent as SubmitEvent).submitter; void submitAddition(submitter instanceof HTMLButtonElement && submitter.value === "continue"); }}>
        ...
        <div className="stock-add-grid">
          {products.length > 0 && (addMode === "new" ? <label>Product name<input autoFocus required value={newProduct.name} onChange={(event) => setNewProduct((current) => ({ ...current, name: event.target.value }))} /></label> : <CustomSelect className="stock-add-product-selector" label="Product name" value={existingProductId} options={[{ value: newItemValue, label: "+ New item" }, ...products.map((product) => ({ value: product.id, label: `${product.name}${product.size ? ` (${product.size})` : ""}` }))]} onChange={(value) => { setError(""); if (value === newItemValue) { setAddMode("new"); setExistingProductId(""); setNewProduct((current) => ({ ...current, name: "" })); } else { setAddMode("existing"); setExistingProductId(value); } }} />)}
