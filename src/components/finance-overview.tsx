...
export function FinanceOverview() {
  ...
  return <div className="finance-page-content">
    <div className="finance-summary-grid">{cards.map(({ title, value, description, icon: Icon, tone }) => { const blueCard = tone === "blue"; return <article className={`stock-summary-card finance-summary-card ${tone}${blueCard ? " stock-value-card" : ""}`} key={title}><span className="stock-summary-title">{title}</span><span className="stock-summary-icon finance-card-icon"><Icon aria-hidden="true" /></span><div className={blueCard ? "stock-value-amount" : "stock-summary-value-row"}>{blueCard ? money(value) : <strong>{money(value)}</strong>}</div><span className={blueCard ? "historical-card-note" : "stock-summary-previous"}>{description}</span></article>; })}</div>
