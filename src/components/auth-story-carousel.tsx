...
import { FileText, LayoutDashboard, MessageCircle, ShoppingCart } from "lucide-react";
...
export function AuthStoryCarousel() {
  ...
  return <div className="auth-carousel">
    ...
    <span className="auth-feature-callout" aria-hidden="true" key={slides[active].title}><MessageCircle className="auth-feature-bubble-outline" /><span className="auth-feature-callout-glyph"><ActiveIcon /></span></span>
    <div className="auth-showcase-caption" aria-live="polite" aria-atomic="true" key={slides[active].title}><h2>{slides[active].title}</h2><p>{slides[active].description}</p></div>
