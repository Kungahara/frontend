...
export function VerificationSent() {
  ...
  return <><header className="verification-heading"><MailCheck aria-hidden="true" /><h1>Check your email</h1><p>Verify your email before you can enter your Kungahara workspace.</p></header><form className="auth-form" onSubmit={resend}><label>Email address<input name="email" type="email" autoComplete="email" defaultValue={initialEmail} required /></label>{message && <p className="form-success">{message}</p>}{error && <p className="form-error" role="alert">{error}</p>}<button className="submit-button" disabled={busy}>{busy ? commonText("sending") : "Resend verification email"}</button></form><p className="switch-auth recovery-link"><Link href="/login">Back to sign in</Link></p></>;
}
...
export function VerifyEmail() {
  ...
  return <div className="verification-heading"><MailCheck aria-hidden="true" /><h1>{error ? "Unable to verify" : loadingText("verifyingEmail")}</h1><p>{error || loadingText("verifyingEmailBody")}</p>{error && <Link className="text-link" href="/verify-email-sent">Request a new link</Link>}</div>;
}
