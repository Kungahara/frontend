...
export function SalesHistoryTable({ navigation }: { navigation?: ReactNode }) {
  ...
  const analysis = useMemo(() => {
    ...
    const start = period === "1Y" ? new Date(year, 0, 1) : period === "1M" ? new Date(monthYear, monthIndex, 1) : new Date(monthYear, monthIndex, selectedWeek.startDay);
    const end = period === "1Y" ? new Date(year + 1, 0, 1) : period === "1M" ? new Date(monthYear, monthIndex + 1, 1) : new Date(monthYear, monthIndex, selectedWeek.endDay + 1);
    const selected = sales.filter((sale) => { const date = new Date(sale.createdAt); return date >= start && date < end; });
    const bucketDates = period === "1Y"
    ...
    const buckets = bucketDates.map((date, index) => {
      const next = period === "1Y" ? new Date(year, index + 1, 1) : period === "1M" ? new Date(monthYear, monthIndex, (weeks[index]?.endDay ?? date.getDate()) + 1) : new Date(date.getFullYear(), date.getMonth(), date.getDate() + 1);
  ...
  }, [sales, period, year, month, weekIndex, weeks]);
  ...
  return <div className="sales-history-view">
    <div className="stock-summary-grid historical-summary-grid">
      <article className="stock-summary-card stock-value-card" aria-label="Money made in selected period"><span className="stock-summary-title">Money made</span><span className="stock-summary-icon stock-value-icon"><WalletCards aria-hidden="true" /></span><div className="stock-value-amount">{money(analysis.total)}</div><span className="historical-card-note">In the selected {period === "1W" ? "week" : period === "1M" ? "month" : "year"}</span></article>
      <article className="stock-summary-card selling-summary-card best" aria-label="Highest sales period"><span className="stock-summary-title">Highest sales {period === "1W" ? "day" : period === "1M" ? "week" : "month"}</span><span className="stock-summary-icon selling-summary-icon"><BadgeDollarSign aria-hidden="true" /></span><div className="stock-summary-value-row historical-card-value"><strong>{money(analysis.top.value)}</strong></div><span className="stock-summary-previous">{analysis.top.label}</span></article>
      <article className="stock-summary-card" aria-label="Items sold"><span className="stock-summary-title">Items sold</span><span className="stock-summary-icon stock-status-icon"><StockStatusIcon /></span><div className="stock-summary-value-row"><strong>{itemsSold}</strong></div><span className="stock-summary-previous">Units sold in this {period === "1W" ? "week" : period === "1M" ? "month" : "year"}</span></article>
      <article className="stock-summary-card" aria-label="Sales recorded"><span className="stock-summary-title">Sales recorded</span><span className="stock-summary-icon historical-count-icon"><ClipboardList aria-hidden="true" /></span><div className="stock-summary-value-row"><strong>{analysis.selected.length}</strong></div><span className="stock-summary-previous">Transactions in this {period === "1W" ? "week" : period === "1M" ? "month" : "year"}</span></article>
    </div>
    {navigation}
    <section className="stock-product-panel sales-product-panel historical-sales-table" aria-labelledby="historical-sales-title">
      <header className="stock-product-toolbar sales-history-toolbar"><h2 id="historical-sales-title">{periodTitle}</h2>
        <div className="sales-history-toolbar-actions"><div className="stock-period-controls">
          ...
          <div className="stock-period-buttons" aria-label="Sales period">{(["1W", "1M", "1Y"] as Period[]).map((value) => <button className={period === value ? "active" : ""} type="button" key={value} onClick={() => setPeriod(value)}>{{ "1W": "Week", "1M": "Month", "1Y": "Year" }[value]}</button>)}</div>
