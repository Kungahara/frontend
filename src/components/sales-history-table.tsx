...
export function SalesHistoryTable({ navigation }: { navigation?: ReactNode }) {
  ...
  return <div className="sales-history-view">
    <div className="stock-summary-grid historical-summary-grid">
      <article className="stock-summary-card stock-value-card" aria-label="Money made in selected period"><span className="stock-summary-title">Money made</span><span className="stock-summary-icon stock-value-icon"><Coins aria-hidden="true" /></span><div className={`stock-value-amount${loading ? " sales-value-loading" : ""}`}>{loading ? "Loading…" : formatRwf(analysis.total)}</div><span className="historical-card-note">In the selected {period === "1D" ? "day" : period === "1W" ? "week" : period === "1M" ? "month" : "year"}</span></article>
      <article className="stock-summary-card selling-summary-card best" aria-label="Highest sales period"><span className="stock-summary-title">Highest sales {period === "1D" ? "hour" : period === "1W" ? "day" : period === "1M" ? "week" : "month"}</span><span className="stock-summary-icon selling-summary-icon"><BadgeDollarSign aria-hidden="true" /></span><div className="stock-summary-value-row historical-card-value"><strong className={loading ? "sales-value-loading" : ""}>{loading ? "Loading…" : formatRwf(analysis.top.value)}</strong></div><span className="stock-summary-previous">{loading ? "Fetching sales" : analysis.top.label}</span></article>
    ...
    <section className="stock-product-panel sales-product-panel historical-sales-table" aria-labelledby="historical-sales-title">
      ...
      <div className="stock-product-table-wrap"><table className="stock-product-table sales-product-table"><thead><tr><th>Date</th><th>Name</th><th>Category</th><th>Size</th><th aria-sort={tableSort?.key === "money" ? tableSort.direction === "asc" ? "ascending" : "descending" : "none"}><MoneySortButton label="Sold for" direction={tableSort?.key === "money" ? tableSort.direction : null} onToggle={() => toggleTableSort("money")} /></th><th aria-sort={tableSort?.key === "quantity" ? tableSort.direction === "asc" ? "ascending" : "descending" : "none"}><MoneySortButton label="Quantity" direction={tableSort?.key === "quantity" ? tableSort.direction : null} onToggle={() => toggleTableSort("quantity")} /></th></tr></thead><tbody className={!loading && !visibleSales.length ? "empty" : ""}>
        ...
        {visibleSales.map((sale) => <tr key={sale.id}><td>{new Intl.DateTimeFormat("en-RW", { dateStyle: "medium", timeStyle: "short" }).format(new Date(sale.createdAt))}</td><td><strong>{sale.productName}</strong></td><td><span className="stock-category-pill">{sale.categoryName}</span></td><td>{sale.size || "—"}</td><td>{formatRwf(Number(sale.total ?? Number(sale.unitPrice) * sale.quantity))}</td><td><span className="stock-quantity-value">{sale.quantity}</span></td></tr>)}
