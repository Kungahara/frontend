...
export function DocumentsOverview() {
  ...
  const [period, setPeriod] = useState<DocumentPeriod>("today");
  ...
  return <div className="documents-page-content">
    ...
    <div className="documents-main-row">
      <section className="documents-library" aria-labelledby="documents-library-title">
        ...
        <div className="documents-library-toolbar"><nav className="sales-view-tabs documents-period-tabs" aria-label="Filter documents by date">{([['today', 'Today'], ['month', 'This month'], ['all', 'All']] as const).map(([value, label]) => <button className={period === value ? "active" : ""} type="button" role="tab" aria-selected={period === value} onClick={() => setPeriod(value)} key={value}>{label}</button>)}</nav></div>
