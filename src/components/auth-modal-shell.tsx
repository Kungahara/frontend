...
export function AuthModalShell({ mode, children, onClose, onModeChange }: { mode: "login" | "signup"; children: React.ReactNode; onClose: () => void; onModeChange: (mode: "login" | "signup") => void }) {
  ...
  return <main className="auth-modal-page" onClickCapture={followAuthSwitch}>
    <section className="auth-card" role="dialog" aria-modal="true" aria-label={mode === "login" ? t("signIn") : t("createAccount")}>
      <div className="auth-story" data-mode={mode}><Brand /><AuthStoryCarousel /></div>
      <div className="auth-form-panel"><button className="auth-close" type="button" aria-label={t("closeAuthentication")} onClick={onClose}><X aria-hidden="true" /></button><div className="auth-modal-scroll"><div className="auth-form-brand"><Brand ariaLabel={t("kungaharaHome")} /></div>{children}</div></div>
    </section>
  </main>;
}
